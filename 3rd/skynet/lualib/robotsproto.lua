...
function host:dispatch(...)
	...
	if header.type then
		...
		if header.type > 10000 then
			header.type = header.type - 10000
			proto = queryproto(c2sSproto, header.type)
			if proto.response then
				result = core.decode(proto.response, content)
			end
			return "RESPONSE", proto.name, result, gen_response(self, proto.response, header_tmp.session,header.type), header.ud
		else
			proto = queryproto(self.__proto, header.type)
